...
@app.route('/save_sensor_data/<owner_id>/<iot_module_id>/<float:temperature>/<float:humidity>/<float:moisture>')
def save_sensor_data(owner_id,iot_module_id,temperature,humidity,moisture):
...
@app.route('/get_sensor_data/<owner_id>/<iot_module_id>')
def get_sensor_data(owner_id,iot_module_id):
    pre_check_data=iot_modules.find({"_id":bson.objectid.ObjectId(iot_module_id),"owner_id":owner_id})
    pre_check_data=[i for i in pre_check_data]
    if len(pre_check_data)>0:
...
@app.route('/get_recent_sensor_data/<owner_id>/<iot_module_id>')
